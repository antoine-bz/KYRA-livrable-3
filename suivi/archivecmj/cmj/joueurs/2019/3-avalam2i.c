...
#define NBSTRAT 3		//déclaration d'une constante 
...
/*Cette fonction a pour but de choisir le coup à jouer*/
/*Pour cela elle est en mesure lancer les fonction de selection de coup grâce à un système de priorité afin de jouer le meillleur coup possible*/
void choisirCoup(T_Position currentPosition, T_ListeCoups listeCoups) {
	...
	int isolement_coup;
	int coup_possible;
	int priorite[NBSTRAT];	//le tableau priorité va stocker tout les coups trouvés issus des différentes stratégie 
	...
	for(i=0;i<NBSTRAT;i++){		
		priorite[i]=-1;		//initialisation du tableau à -1 (aucun coup rentré)
	}
	voisins=0;	//initialisation des constante pour traitement plus tard
	taille=5;
	...
	// Si je peux gagner une tour de 5(porter un coup critique [cf livrable 2]), je fait ce coup prioritairement
		if ( (currentPosition.cols[o].couleur == myColor) //si la colonne d'origine est de ma couleur
		&& (currentPosition.cols[d].nb+currentPosition.cols[o].nb == 5) && (currentPosition.cols[d].couleur != myColor)) { //et que l'on fait une tour sur un pion de l'adversaire 

			ecrireIndexCoup(i);	//on écrit le coup
			printf("tour de 5 prise par le coup %d : %d ---> %d \n",i,o,d);			
			return; // on quitte la fonction, car ce coup est prioritaire!!!
		}

		coup_possible=isolement(&currentPosition,&listeCoups,i,&taille);	//lancement de la fonction d'isolement 
		if(coup_possible!=1)priorite[0]=coup_possible;//on range le coup seulemet si un coup n'a pas été trouvé

		coup_possible=empileradverse(&currentPosition,&listeCoups,i,&voisins);//lancement de la fonction d'empilement
		if(coup_possible!=-1)priorite[1]=coup_possible;

		coup_possible=empileradverse2(&currentPosition,&listeCoups,i);	//lancement de la seconde fonction 'empilement'
...
//erreur critique est un pillier de notre stratégie; elle nous facilite réellement toute la stratégie
//Son but est de "prévoir le coup de l'adversaire" c'est à dire déterminer si le joueur ne pourra pas créer une tour de 5 ou isoler un de ses pions après notre coup!!!
...
int nbvoisin(T_Position * Position,octet pion){
    ...
    for(i=0;i<Voisin.nb;i++){		//on parcours les voisins du pions soumis à la fonction
        if(Position->cols[Voisin.cases[i]].nb!=0 && Position->cols[Voisin.cases[i]].nb!=5)compteur++;
        //s'ils la hauteur des tours est différente de 0(case vide) et différente de 5(tour immobile) on considère le pion comme un voisin
...
//fonction d'engagement de début de partie n'ayant pue etre utilisée mais tout de même présentée
/*int sixpremierstours(T_Position * Position, T_ListeCoups * listeCoups,int j){		//stratégie abandonnée
	int i=0;
	int nbcoups=0; //compte le nombre de case vide pour d
	octet o,d;
	...
	if(nbcoups<6){
		switch(nbcoups){
			/*case 0: if (o==26 && d==27 )return j;		//26->27: coup 151
			break;
			case 1: if (o==27 && d==19 )return j;		//27->19: 155 
			break;
			case 2: if (o==18 && d==11 )return j;
			break;
			case 3: if (o==18 && d==26 )return j;
			break;
			case 4: if (o==27 && d==35 )return j;
			break;
			case 5: if (o==26 && d==34 )return j;
			break;
			default:
			break;
		}
