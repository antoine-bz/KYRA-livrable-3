/********** Moteur de tournoi : duel-instantane ************/
/* 
...
void * thread_partie(void * i) {
	...
	while(l.nb > 0) {
		...
		if (p.trait == JAU) {
			...
			// TODO: sem_wait nouveau
			// Faire duel2 ? 
			// sleep(delaiChoixCoupJaune); // plus de mécanisme de temps 
			printf1("attente coup du joueur %d ...\n", partie.pidJ);
			sem_wait(joueurs.tab[partie.iJ].pSem);  
			printf0("... ok\n");
		}
		else {
			...
			// On attend une durée fonction du joueur au trait 
			// sleep(delaiChoixCoupRouge);	// plus de mécanisme de temps 
			printf1("attente confirmation par joueur %d ...\n", partie.pidR);
			sem_wait(joueurs.tab[partie.iR].pSem);  
			printf0("... ok\n");
		}
